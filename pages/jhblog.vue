...
<template>
	<div>
		<section class="h-screen w-screen fixed z-10">
			<IconsMandala3 class="fill-brand-champagne-gold opacity-60" style="width: 150%" />
		</section>
		...
		<div
			class="relative grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-10 lg:gap-20 xl:gap-16 w-screen h-auto mb-5 font-domainesemibold lg:font-domainemedium px-2 md:px-4 lg:px-8 z-20 text-brand-grey bg-white"
		>
			<div v-for="blog in blogs" :key="blog._id">
				...
				>
					<span class="h-40 bg-brand-champagne-gold rounded-xl">
						<h1 class="m-1 p-1 text-xl sm:text-2xl md:text-3xl">
							{{ blog.title }}
						</h1>
