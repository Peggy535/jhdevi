...
<template>
	<div>
		...
		<div class="relative flex h-auto w-screen font-domainemedium text-brand-champagne-gold z-20">
			<h1 class="text-8xl m-2 p-2 my-20">JH Blog</h1>
		</div>
		<div
		...
		>
			<div v-for="blog in blogs" :key="blog._id">
				<article
					class="grid col-span-1 min-h-min border border-brand-champagne-gold rounded-2xl transition duration-500 ease-in-out hover:shadow-lg hover:shadow-brand-champagne-gold group hover:scale-105 bg-brand-light-cream"
				>
					<span class="h-40 bg-brand-champagne-gold rounded-xl">
						<h1 class="m-1 p-1 text-3xl sm:text-2xl md:text-3xl lg:text-4xl">
							{{ blog.title }}
						</h1>
						<p class="text-sm m-1 p-1">{{ blog.minRead }} min read</p>
					</span>
					<span class="relative h-48 sm:h-60 rounded-2xl bg-brand-light-cream">
						<p class="m-1 p-1">{{ new Date(blog.publishedDate).toLocaleDateString() }}</p>
